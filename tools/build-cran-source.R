...
output_dir <- normalizePath("~", mustWork = TRUE)
...
staging_parent <- tempfile("QCA-cran-source-")
...
if ("Suggests" %in% colnames(staged_description)) {
    ...
    suggests <- suggests[!grepl("^testthat(\\s*\\(|$)", suggests)]

    if (length(suggests) == 0) {
        staged_description <- staged_description[, colnames(staged_description) != "Suggests", drop = FALSE]
    } else {
...
staged_description <- staged_description[
    ,
    colnames(staged_description) != "Config/testthat/edition",
    drop = FALSE
]
...
build_status <- system2(
    ...
    c("CMD", "build", "--no-build-vignettes", shQuote(staging_root)),
    stdout = "",
    stderr = ""
)
...
tarball <- file.path(getwd(), paste0(package_name, "_", description[[1, "Version"]], ".tar.gz"))
target <- file.path(output_dir, basename(tarball))

if (normalizePath(dirname(tarball), mustWork = TRUE) != normalizePath(output_dir, mustWork = TRUE)) {
