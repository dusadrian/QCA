`minimize` <- function(
    ...
    # when excluding remainders and number of conditions > 7, the "cases" component
    # is larger than the "indexes" component (there are more 0s in the "OUT" column)
    ...
    if (incl.rem) {
        ...
        if (method == "QMC") {
            ...
            hybrid <- !isFALSE(dots$hybrid)

            if (!isFALSE(dots$gurobi) && !hybrid) {
    ...
    if (SA & 3^length(noflevels) < 2^31) {
        ...
        output$SA <- .Call(
            "C_getSA",
            p.sol$solution.list[[1]],
            p.sol$reduced$expressions,
            as.integer(noflevels),
            as.integer(mbaseexpr),
            inputt,
            as.integer(mbaseplus),
            as.integer(mbase),
            PACKAGE = "QCA"
        )
            ...
            for (c.s in seq(ncol(c.sol$sol.matrix))) {
                ...
                for (p.s in seq(ncol(p.sol$sol.matrix))) {
                    ...
                    if (all(unlist(lapply(output$SA, is.null))) | isoliscsol) {
                        ...
                        # rownames(expressions) <- admisc::writePIs(expressions, mv = mv, collapse = collapse)
