`minimize` <- function(
    ...
    # argument dir.exp should NOT be defaulted to "", because that might be
    # interpreted as an universal don't care
    ...
    if (is.null(include)) {
        ...
        # this solves a potential situation when users set include = NULL
        # and dir.exp to "" (2020.03.18 Paola Scuglio)
    }
    ...
    if (ttinput) { # input already is a tt
        tt <- input

        ttargs <- setdiff(names(formals(truthTable)), c("show.cases", "use.labels"))
        # ttargs <- names(formals(truthTable))
        ...
        if (length(outcome) > 1) {
            # metacall[[1]] <- as.name("minimizeLoop")
            # return(suppressWarnings(eval(metacall, envir = parent.frame())))
            return(do.call("minimizeLoop", as.list(metacall)[-1], envir = parent.frame()))
        }

        outcome.copy <- outcome.name <- outcome
        indata <- input # important before altering the outcome, if multi-value
        ...
        # indata <- input # data is already altered in outcome value, if initially multi-value
    ...
    ### bug fix 2016.08.05 reported by Carsten Schneider and Nena Oana
    indata <- tt$initial.data[, match(colnames(recdata), colnames(tt$initial.data)), drop = FALSE]
    ###

    use.letters <- tt$options$use.letters
    show.cases <- show.cases | tt$options$show.cases # it can be forced here, even if truthTable() doesn't have it
    ...
    # return(list(data=recdata, outcome=outcome, conditions=conditions, dir.exp=dir.exp))
    ...
    # if (!is.null(tt$options$exclude)) {
    #     if (identical(include, "")) {
    #         include <- "?"
    #     }
    #     else if (!is.element("?", include)) { # include is already split
    #         include <- c("?", include)
    #     }
    # }

    pos.incl <- unique(c(explain, include)) # here "include" may contain contradictions; missings are irrelevant as they were already erased
    ...
    # fixes bug reported by Ben Gibson, 17.05.2016
    rownames(pos.matrix) <- drop(pos.matrix %*% mbase) + 1
    # was rownames(pos.matrix) <- tt$indexes[subset.tt]
    ...
    if (nrow(neg.matrix) == 0 & incl.rem & !is.element("causalChain", names(dots))) { # & method == "QMC"
        admisc::stopError(
            paste(
    ...
    # pos.matrix needs to be unaltered for incl.rem
    ...
    if (incl.rem) {
        ...
        #     admisc::stopError("There are no remainders to include, all configurations are observed.", ... = ...)
        # }

        # contradictions should no longer be part of the formal explained matrix
        # since they are treated as remainders (by being taken out of the excluded matrix)
        pos.matrix <- inputt
        ...
        if (method == "QMC") {
            ...
            # ))
            # dyn.load("~/Lucru/_R/QCA/src/QCA.so")
            # .Call("C_Cubes", aa)
            ...
            expressions <- .Call("C_Cubes", list(
                            ...
                            PACKAGE = "QCA")
            # dyn.unload("~/Lucru/_R/QCA/src/QCA.so")
            # return(expressions)
# prim procurorul parchetului de pe langa tribunalul suceava
        }


        ### time and memory being spent to copy this matrix just to send it to getSolution()...!
    ...
    # output$PIchart <- structure(list(p.sol$mtrx), class = "QCA_pic")
    ...
    # output$options$warn1conf <- ifelse(nrow(pos.matrix) == 1 & !incl.rem, TRUE, FALSE)
    mtrx <- p.sol$mtrx[p.sol$all.PIs, , drop = FALSE]

    ### to rewrite in C, it takes a LOT of time
    #
    ...
    if (SA & 3^length(noflevels) < 2^31) {
        ...
        ### bug fix 13.09.2017 when using remainders on temporal QCA:
        ### the temporal column(s) get removed and that influences mbaseplus, now mbaseexpr
        mbaseexpr <- rev(c(1, cumprod(rev(noflevels[is.element(conds, colnames(p.sol$reduced$expressions))] + 1))))[-1]

        # output$SA <- vector(mode = "list", length = ncol(p.sol$sol.matrix))
        # return(list(p.sol=p.sol, mbaseexpr=mbaseexpr, noflevels=noflevels, inputt=inputt, mbaseplus=mbaseplus, mbase=mbase))

        output$SA <- lapply(p.sol$solution.list[[1]], function(x) {
            # TO DO: calculate SAs in C, should be much faster...!
            ...
            if (all(is.null(temp))) {
                ...
                if (length(temp) == 0) {
                    ...
                    # bug fix 28.02.2020 as 3^length(noflevels) < 2^31 in the above if()
                    # getRow() expects integers and numbers greater than 2^31 become NAs
                    # when coerced to integers
        ...
        # return(list(dir.exp = dir.exp, include = include, c.sol = c.sol, p.sol = p.sol,
        #             mv = mv, inputt = inputt, row.dom = row.dom,
        #             rownms=rownms, all.sol=all.sol, conditions = conditions,
        #             noflevels = noflevels, output = output, pos.matrix = pos.matrix,
        #             exclude = exclude, collapse = collapse))


        if (!is.null(dir.exp) & !identical(include, "")) {
            if (!identical(c.sol$solution.list, NA)) {

            # return(list(data = recdata, outcome = outcome, conditions = conditions, noflevels = noflevels, dir.exp = dir.exp))
            ...
            # return(dir.exp)
            # return(list(dir.exp = dir.exp, cexpressions = c.sol$expressions,
            #     csol.matrix = c.sol$sol.matrix, pexpressions = p.sol$expressions,
            #     psol.matrix = p.sol$sol.matrix, SA = output$SA, noflevels = as.integer(noflevels)))
            # bla <- .Call("C_getEC", aa[[1]], aa[[2]], aa[[3]], aa[[4]], aa[[5]], aa[[6]], aa[[7]])
            ...
            for (c.s in seq(ncol(c.sol$sol.matrix))) {
                ...
                for (p.s in seq(ncol(p.sol$sol.matrix))) {
                    ...
                    if (all(unlist(lapply(output$SA, is.null))) | isoliscsol) {
                        ...
                        # expressions <- isols[[index]]
                        # rownames(expressions) <- admisc::writePrimeimp(expressions, mv = mv, collapse = collapse)
                    ...
                    # which() also fixes a possibility that truthTable() doesn't correctly subsets cases
                    # bug fix 2016.06.10 Carsten and Nena
                    for (l in seq(length(expr.cases))) {
