`superSubset` <- function(
    ...
    # Unlike pof() where the outcome can be a vector, here it MUST be a column name
    # TO DO: perhaps extend to vectors as well, but be careful if the input is an
    # existing object with the same name as one of the columns.
    # Once going for recognizing unquoted SOP expressions, things becomes tricky
    # if (!identical(class(outcome), "character")) {
    #     admisc::stopError("The outcome should be a column name.", ... = ...)
    # }
    ...
    # just to make sure we have at least 2 levels
    noflevels[noflevels == 1] <- 2 # odata cu noul algoritm s-ar putea sa NU MAI FIE NECESAR !!
    ...
    attr(mins, "conditions") <- conditions # the future "coms" component
