...
#define XCALLOC(ptr, nr)\
  ...
  else if(!(ptr = (void *) R_Calloc((size_t)(nr), __typeof__(*ptr)))) {\
    REprintf("calloc of %d bytes failed on line %d of file %s\n",\
...
#define XMALLOC(ptr, nr)\
  ...
  else if(!((void *) ptr = (void *) R_Calloc((size_t)(nr), __typeof__(*ptr)))) {\
    REprintf("malloc of %d bytes failed on line %d of file %s\n",\
             (size_t) nr * sizeof(*ptr), __LINE__, __FILE__);\
  }
#endif
...
#define XREALLOC(ptr, nr)\
  ...
  else if(!((void *) ptr = (void *) R_Realloc(ptr, (size_t)(nr), __typeof__(*ptr)))) {\
    REprintf("realloc of %d bytes failed on line %d of file %s\n",\
             (size_t) nr * sizeof(*ptr), __LINE__, __FILE__);\
  }
#endif
...
#define STRALLOCCOPY(nstr, ostr)\
  {nstr = (char *) R_Calloc((size_t) (strlen(ostr) + 1), char);\
   strcpy(nstr, ostr);}
#endif
