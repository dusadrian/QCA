...
/* QCA internal build: keep the existing order instead of running MDO/COLAMD. */
...
/* ---------------------------------------------------------------------------------- */
/* QCA internal build: model/file I/O and parameter-file support are disabled. */
...
STATIC int get_constr_class(lprec *lp, int rownr)
{
  ...
  for(; elmnr < elmend; elmnr++) {
    ...
    if(fabs(a-1.0) < lp->epsvalue)
      ...
      /* Includes negative integer-valued coefficients */
    }
  }
...
STATIC MYBOOL verify_basis(lprec *lp)
{
  ...
  for(i = 1; i <= lp->rows; i++) {
    ...
    if (buttrey_thing > 0) {
        Rprintf("lp lib: We're down here now.\n");
    }
  ...
  for(i = 1; i <= lp->sum; i++) {
    ...
    if (buttrey_thing > 0) {
        Rprintf("lp lib: About to return.\n");
    }
  result = (MYBOOL) (ii == 0);
...
STATIC int find_int_bbvar(lprec *lp, int *count, BBrec *BB, MYBOOL *isfeasible)
{
  ...
  for(ii = 1; ii <= lp->columns; ii++) {
    i = get_var_priority(lp, ii);
    if(!is_int(lp, i))
      continue;

    i = lp->rows + i;
    if(BB->lowbo[i] == BB->upbo[i])
      continue;
    if(solution_is_int(lp, i, FALSE))
      continue;

    (*count)++;
    frac = modf(lp->solution[i], &whole);
    frac = fabs(frac - 0.5);
    if((bestvar == 0) || (frac < bestgap)) {
      bestgap = frac;
      bestvar = i;
    }
  }
...
STATIC BBPSrec *init_pseudocost(lprec *lp, int pseudotype)
{
  ...
  newitem = (BBPSrec*) calloc(1, sizeof(*newitem));
  if(newitem == NULL)
    return(NULL);
