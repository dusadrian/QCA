...
Rboolean redundant(
    ...
    while (i < limit && !redundant) {
        ...
        if (
            chart_dominated != NULL && !*chart_dominated &&
            p_pichart_pos != NULL && candidate_coverage != NULL &&
            solver_active != NULL && solver_active[pi_index]
        ) {
            Rboolean coverage_subset = true;
            for (int w = 0; w < pichart_words; ++w) {
                if (
                    candidate_coverage[w] &
                    ~p_pichart_pos[pi_index * pichart_words + w]
                ) {
                    coverage_subset = false;
                    break;
                }
            }
            *chart_dominated = coverage_subset;
        }
